...
func (s *UserService) GetProfile(ctx context.Context, userID uint) (*models.UserProfileResponse, error) {
	...
	// Batch fetch all streaks for user's habits
	var habitIDs []uint
	for _, h := range habits {
		habitIDs = append(habitIDs, h.ID)
	}
	allStreaks, err := s.streakRepo.FindByHabitIDs(ctx, habitIDs) // You need to implement this batch method
	if err != nil {
		return nil, err
	}

	// Batch fetch all check-ins for these streaks
	var streakIDs []uint
	for _, st := range allStreaks {
		streakIDs = append(streakIDs, st.ID)
	}
	allCheckIns, err := s.checkInRepo.FindByStreakIDs(ctx, streakIDs) // You need to implement this batch method
	if err != nil {
		return nil, err
	}

	// Use new helper functions that filter in-memory
	overview := s.calculateOverviewStatsWithPrefetch(user, habits, achievements, allStreaks, allCheckIns)
	streakInsights := s.calculateStreakInsightsWithPrefetch(habits, allStreaks)
	consistencyChart := s.calculateConsistencyChartWithPrefetch(habits, allStreaks, allCheckIns, 30)
	topHabits := s.calculateTopHabitsWithPrefetch(habits, allStreaks, allCheckIns)
	...
	nextMilestone := s.predictNextMilestoneWithPrefetch(habits, allStreaks)
...
func (s *UserService) calculateOverviewStatsWithPrefetch(user *models.User, habits []models.Habit, achievements []models.Achievement, allStreaks []models.HabitStreak, allCheckIns []models.HabitCheckIn) models.OverviewStats {
	now := time.Now()
	...
	habitIDSet := make(map[uint]struct{})
	for _, h := range habits {
		habitIDSet[h.ID] = struct{}{}
	}
	totalCheckIns := 0
	for _, checkIn := range allCheckIns {
		for _, streak := range allStreaks {
			if streak.ID == checkIn.StreakID {
				if _, ok := habitIDSet[streak.HabitID]; ok {
					totalCheckIns++
				}
				break
			}
	...
	// Consistency calculations (all time, weekly, monthly)
	overallConsistency := s.calculateConsistencyForPeriodWithPrefetch(habits, allStreaks, allCheckIns, 0)
	weeklyConsistency := s.calculateConsistencyForPeriodWithPrefetch(habits, allStreaks, allCheckIns, 7)
	monthlyConsistency := s.calculateConsistencyForPeriodWithPrefetch(habits, allStreaks, allCheckIns, 30)
...
func (s *UserService) calculateConsistencyForPeriodWithPrefetch(habits []models.Habit, allStreaks []models.HabitStreak, allCheckIns []models.HabitCheckIn, days int) float64 {
	if len(habits) == 0 {
		return 0
	}
	now := time.Now().UTC()
	...
	for _, habit := range habits {
		...
		for _, streak := range allStreaks {
			if streak.HabitID != habit.ID {
				continue
			}
			for _, checkIn := range allCheckIns {
				if checkIn.StreakID == streak.ID && checkIn.CheckInDate.After(periodStart) && checkIn.CheckInDate.Before(now) {
					totalActual++
				}
			}
		}
...
func (s *UserService) calculateStreakInsightsWithPrefetch(habits []models.Habit, allStreaks []models.HabitStreak) models.StreakInsight {
	...
	return models.StreakInsight{
		...
		AverageStreakLength:  math.Round(avgStreak*100) / 100,
		ActiveStreaksCount:   activeCount,
	}
}

func (s *UserService) calculateConsistencyChartWithPrefetch(habits []models.Habit, allStreaks []models.HabitStreak, allCheckIns []models.HabitCheckIn, days int) []models.ConsistencyDataPoint {
		...
		for _, habit := range habits {
			if habit.IsActive && habit.CreatedAt.Before(date.Add(24*time.Hour)) {
				...
				for _, streak := range allStreaks {
					...
					for _, checkIn := range allCheckIns {
						if checkIn.StreakID == streak.ID && checkIn.CheckInDate.Truncate(24*time.Hour).Equal(date) {
							totalCheckIns++
							break
						}
					}
...
func (s *UserService) calculateTopHabitsWithPrefetch(habits []models.Habit, allStreaks []models.HabitStreak, allCheckIns []models.HabitCheckIn) []models.HabitPerformance {
	...
	for _, habit := range habits {
		...
		performances = append(performances, models.HabitPerformance{
			HabitID:         habit.ID,
...
func (s *UserService) predictNextMilestoneWithPrefetch(habits []models.Habit, allStreaks []models.HabitStreak) *models.AchievementResponse {
	for _, habit := range habits {
		for _, streak := range allStreaks {
			if streak.HabitID == habit.ID && streak.Status == "active" {
